$(function () {
    ...
    touch.on(tabpanel, 'tap', function (ev) {
        ...
        $(this).addClass("c-base").siblings('dd').removeClass("c-base");
        ...
        if (div === 0 && module !== 'order') {
            ...
            initParam();
            filterObj[_ft] = $(this).data('filter');
            unLockDropload();
            filterFn(dropload, 1);
    ...
    while (len--) {
        if (module === 'order') {
            dom += '<div class="myorder-item">' +
                ...
                '<span class="price fr"><em>￥</em>' + list[len].orderSum + '</span>' +
                '</div>' +
                '<a href="/member/order/' + list[len].orderNo + '">' +
                '<h3 class="myorder-item-title">' + list[len].orderInfo + '</h3>';
            if (list[len].orderType !== 'shop') {
                dom += '<p>' + (list[len].orderStatus === "2" ? '已使用' : '未使用') + '( ' + list[len].startTime.substring(0, 10) + ' 使用)</p>' +
                    '<p><i class="font-icon fr icon-iconfont-jiantou"></i>' + list[len].amount + '张</p>';
            } else {
                dom += '<p>'+(list[len].sendType === "0" ? '快递' : '自提')+'</p>'
                    + '<p>'+list[len].linkAddr+'</p>'
                    + '<p><i class="font-icon fr icon-iconfont-jiantou"></i>' + list[len].amount + '件</p>';
            }
            dom += '</a></div><div class="page-line"></div>';


            console.log(list[len].createTime, list[len].orderStatus, payStatus(list[len].orderStatus, list[len].orderType))
            ...
            switch (module) {
                ...
                case 'guide':
                    dom += '<div class="page-list-info">' +
                        ...
                        guideLevel(list[len].level) +
                        '</h3>' +
                        '<p class="page-list-article">服务城市：' + list[len].city + '</p>' +
                        '<p class="page-list-explian">' + tag + '</p>' +
                        '</div></a></li>';
                    break;
                case 'strategy':
                ...
                case 'rentCar':
                    dom += '<div class="page-list-info">' +
                        ...
                        '<span class="price fr"><em>￥</em><strong>' + (list[len].salesPrice ? (+list[len].salesPrice).toFixed(2) : '0.00') + '</strong>/天</span>' +
                        '</p>' +
                        '</div></a></li>';
                    break;
                case 'shop':
                    var _price = list[len].currentPrice ? '<span class="price"><em>￥</em><strong>' + (list[len].currentPrice).toFixed(2) + '</strong></span>' : ''; 
